...
class auditoriasController
{
	public static function cierreAuditoria($id,$fecha){
		return empresaDao::cierreAuditoria($id,$fecha);
	}
	...
	public static function listadoRolAuditoriaAnclaAsociada($idempresa,$idempresaAsociada){
		$objAuditoria=auditoriaDao::listadoRolAuditoriaAnclaAsociadaPropiasAsociada($idempresa,$idempresaAsociada);
		...
		echo   "<table  class='table table-bordered table-responsive-lg'>
					...
					<tbody>
						...
						echo"
						<tr role='row' class='even'>
						<td><a href='".MODULE_APP_SERVER."auditorias/verFichaRolAncla.php?id=".$objAuditoria[$key]->getIdauditoria()."&vista=Ancla' class='dropdown-item'><i class='fa fa-eye fa-2x'></i> </a></td>
							";
							switch ($objAuditoria[$key]->getEstado_auditoria()) {
	...
	public static function listadoRolAuditoriaAncla($idempresa){
		$objAuditoria=auditoriaDao::listadoRolAuditoriaAncla($idempresa);
	...
	public static function listadoRolAuditoriaAsociada($idempresa){
		$objAuditoria=auditoriaDao::listadoRolAuditoriaAsociada($idempresa);
		...
		echo   "<table  class='table table-bordered table-responsive-lg'
					...
					<tbody>
						...
						echo"
						<tr role='row' class='even'>
						<td><a href='".MODULE_APP_SERVER."auditorias/verFichaRolAnclaAsociada.php?id=".$objAuditoria[$key]->getIdauditoria() ."&vista=Asociada' class='dropdown-item'><i class='fa fa-eye fa-2x'></i> </a></td>
							";
							switch ($objAuditoria[$key]->getEstado_auditoria()) {
	...
	public static function listadoSimpleAuditoriaEmpresa($idempresa){
		$objAuditoria=auditoriaDao::listadoCompuestoAuditoriaEmpresa($idempresa);
	...
	public static function listadoSimpleAuditoriaAuditor($idempresa){
		$objAuditoria=auditoriaDao::listadoCompuestoAuditoriaAuditor($idempresa);
		...
		echo   "<table  class='table table-bordered table-responsive-lg '
					...
					<thead>
						...
						<th  class='parrafo'><li class='fa fa-cog'></li>  Acciones
							...
							<th  class='parrafo'><li class='fa fa-globe'></li>  Fecha auditoría
							</th>
	...
	public static function listadoAuditoriasConsulta($estado,$valor){
		$objAuditoria=auditoriaDao::listadoAuditoriasConsulta($estado,$valor);
		//print_r($objAuditoria);
		//printf_r($objAuditoria);
		if($objAuditoria!= false){
		echo   "<table  class='table table-bordered table-responsive-lg '
												...
												<div >
													<div  style=''>
														...
														echo"
															...
															echo    "<a  href='#".$GrupoPlantilla[$key]->getIdauditoria_item()."' title='".$GrupoPlantilla[$key]->getIdauditoria_item()."' id='validarElemento2'
	...
	public static function listadoGrupoAuditoriaItemSeleccion($idauditoria){
		...
		// 			<strong>".$objPlantilla->getEtiqueta_item_grupo_plantilla()."</strong>
		// 		</div>";
		echo    "<div id='".$GrupoPlantilla[$key]->getIdauditoria_item()."'></div>";
		echo    "<table   class='table table-responsive-md table-white-space table-bordered row-grouping display no-wrap icheck table-middle dataTable'
					...
					<tbody>";
						...
						<tr role='row' class='even'>
							...
							<td class='reorder sorting_1'>
							<textarea readonly rows='4' cols='110'>".$objPlantilla->getDescripcion_item_grupo_plantilla()."</textarea>
							</td>
							<td class='reorder sorting_1'>
							    <div class='form-group'>
									<select id='estado".$GrupoPlantilla[$key]->getIdauditoria_item()."' name='estado".$GrupoPlantilla[$key]->getIdauditoria_item()."' class='form-control'>";
							if($GrupoPlantilla[$key]->getEstado_auditoria_item() == 0){
								echo   "<option value='d'>NO SELECCIONADO</option>
								        <option value='a'>SELECCIONAR</option>";
							}else{
								echo   "<option value='a'>SELECCIONAR</option>
								        <option value='d'>NO SELECCIONAR</option>";
							}
							echo "
							    ...
							    <div class='form-group col-sm-12 col-md-2 text-center mt-2'>
							    	<a href='#' title='".$GrupoPlantilla[$key]->getIdauditoria_item()."' id='seleccionElemento' class='btn capa round text-white waves-effect waves-light'> <i
							    			class='fa fa-check fa-2x'></i>&nbsp; Validar</a>
							    </div>
							</td>
							";
							...
							// if($GrupoPlantilla[$key]->getEstado_auditoria_item() == 0){
							// 	echo "<td class='reorder sorting_1'><a id='seleccionElemento' href='#".$GrupoPlantilla[$key]->getIdauditoria_item()."' title='d' class='dropdown-item'><i class='fa fa-minus-circle fa-2x'></i> </a></td>";
								
							// }else{
							// 	echo "<td class='reorder sorting_1'><a id='seleccionElemento' href='#".$GrupoPlantilla[$key]->getIdauditoria_item()."' title='a' class='dropdown-item'><i class='fa fa-check fa-2x'></i> </a></td>";
		...
		echo   "<table id='' class='table table-responsive-md table-white-space table-bordered row-grouping display no-wrap icheck table-middle dataTable'
					...
					<thead>
						<tr role='row'>
							...
							<th class='sorting_disabled'  > Fecha de Auditoria
							</th>
	...
	public static function listadoGrupoAuditoriaItemPlanAccionPDF($idauditoria){
		$idplanAccion = auditoriaDAO::objPlanAccion($idauditoria);
		$GrupoPlantilla = auditoriaDao::listadoGrupoAuditoriaItemPlanAccion($idplanAccion);
		//( $GrupoPlantilla);
	...
	public static function listadoModeloStatus($idauditoriaAncla){
		$objAuditoria = auditoriaDAO::listadoModeloStatus($idauditoriaAncla);
		
		if($objAuditoria!= false){
			echo   "<table  class='table table-bordered table-responsive-lg'>
						<thead>
							<tr role='row'>
							    <th  ><li class='fa fa-chart-line'></li>  CIUDAD</th>
							    <th ><li class='fa fa-cog'></li>  NOMBRE</th>
								<th  ><li class='fa fa-money-check'></li> ESTATUS</th>
								<th  ><li class='fa fa-barcode'></li> FECHA AUDITORIA</th>
								<th  ><li class='fa fa-money-check'></li> FECHA PLAN DE ACCION</th>
								<th  ><li class='fa fa-money-check'></li> % DE CUMPLIMIENTO</th>
							</tr>
						</thead>
						<tbody>";
							for ($i=0; $i < count($objAuditoria); $i++) { 
							echo"
							<tr role='row' class='even'>
								<td>".$objAuditoria[$i][1]."</td>
								<td>".$objAuditoria[$i][0]."</td>";
                                switch ($objAuditoria[$i][3]) {
									case 'PRO02':
									echo " <td><span class='badge badge-default badge-info'>EN PROGRESO </span></td>";
									break;
								...
								echo "
								<td>".$objAuditoria[$i][2]."</td>";
                                $plan = auditoriaDAO::validarPlanModeloEstatus($objAuditoria[$i][4]);
								if(intval($plan)  == 0){
									if($objAuditoria[$i][3]  == 'PRO13'){
										echo "<td> NA </td>";
									}
									else {
										echo "<td>  </td>";
								...
								$keyAuditoria=$objAuditoria[$i][4];
								$cantidadItemsCumple=auditoriaDAO::numeroItemsPorEstadoEnAuditoria($keyAuditoria,2);
								$cantidadItemsNoCumple=auditoriaDAO::numeroItemsPorEstadoEnAuditoria($keyAuditoria,3);
								$totalItemCumpleNoCumple=$cantidadItemsCumple + $cantidadItemsNoCumple;
								$porcentaje = 0;
								if($objAuditoria[$i][3]  != 'PRO02'){
									$porcentaje=round((100* $cantidadItemsCumple/$totalItemCumpleNoCumple),0);
								}
								echo "
								<td>" .$porcentaje ."%</td>
							</tr>";
	...
	public static function estadoModeloStatus($codigo,$idauditoriaAncla){
		return auditoriaDAO::estadoModeloStatus($codigo,$idauditoriaAncla);
	}
	public static function estadoModeloStatusTotal($idauditoriaAncla){
		return auditoriaDAO::estadoModeloStatusTotal($idauditoriaAncla);
	}
}
?>
